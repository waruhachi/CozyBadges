...
-(void)displayAlert {
    ...
    [alert displayWithCompletion:^void(UIColor *pickedColor) {
        ...
        NSString *path = [NSString stringWithFormat:jbroot(@"/User/Library/Preferences/%@.plist"), self.specifier.properties[@"defaults"]];
...
-(NSString *)previewColor {
    NSMutableDictionary *_prefs = [[NSMutableDictionary alloc] initWithContentsOfFile:jbroot(@"/var/mobile/Library/Preferences/com.noisyflake.cozybadgesprefs.plist")];
    NSString *color = [_prefs valueForKey:[self.specifier propertyForKey:@"key"]];

    if (color == nil) {
        NSMutableDictionary *_defaultPrefs = [[NSMutableDictionary alloc] initWithContentsOfFile:jbroot(@"/Library/PreferenceBundles/CozyBadgesPrefs.bundle/defaults.plist")];
